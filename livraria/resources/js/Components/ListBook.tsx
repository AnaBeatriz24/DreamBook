...
export default function ListBook(props){
    ...
    let body = props.data.data;

    const dataAction = (item:string, id:number)=>{
        switch(item){
            case 'Editar':
                return(
                    <Link href={route('books.search', [id])}  className="inline-flex items-center px-4 py-2 bg-amber-900 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-amber-800 focus:outline-none focus:ring-offset-2 transition ease-in-out duration-150 " type={"submit"}>{item}</Link>
                )
            case 'Deletar':
                const deleteBook={
                    routePost: "book.destroy",
                    item: "livro",
                    id:id
                }
                return(
                    <ComponentDelete routePost={deleteBook.routePost} item={deleteBook.item} id={deleteBook.id}/>
                )
        }
    }
